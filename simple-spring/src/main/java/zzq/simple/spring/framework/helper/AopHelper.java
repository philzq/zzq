...
import zzq.simple.spring.framework.proxy.Proxy;
import zzq.simple.spring.framework.proxy.ProxyFactory;
import zzq.simple.spring.framework.util.ClassUtil;
...
public final class AopHelper {
    ...
    static {
        try {
            ...
            throw new RuntimeException("aop failure", e);
        }
    }
    ...
    private static Map<Class<?>, List<Proxy>> createTargetMap(Map<Class<?>, Set<Class<?>>> aspectMap) throws Exception {
        ...
        for (Map.Entry<Class<?>, Set<Class<?>>> proxyEntry : aspectMap.entrySet()) {
            ...
            for (Class<?> targetClass : targetClassSet) {
                //切面对象
                Proxy aspect = (Proxy) aspectClass.getConstructor().newInstance();
